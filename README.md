...
Docker 실습용 단축 URL 서비스입니다.
**컨테이너 2개, 네트워크 2개, 볼륨 1개**로 실무에서 자주 쓰는 격리 패턴을 학습합니다.

---

## 학습 목표

- Dockerfile을 직접 작성하고 이미지 빌드
- Docker Hub에 push & pull
- Docker 네트워크 2개로 서비스 격리 (`--internal`로 DB 외부 노출 차단)
- 볼륨으로 DB 데이터 영속화
- 컨테이너 간 통신 (서비스 이름을 DNS로 사용)
...
├── app/
│   ├── __init__.py
│   ├── main.py              # FastAPI 앱 (수정 X)
│   └── templates/
│       └── index.html
├── postgres/
│   └── init.sql             # DB 스키마 (postgres가 자동 실행)
├── requirements.txt
├── Dockerfile.solution      # 정답 (직접 작성 후 비교용)
├── .dockerignore
└── README.md                # 이 파일
```
